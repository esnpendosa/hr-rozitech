...
const ROOT_METADATA: Record<AppLocale, { title: string; description: string }> = {
  id: {
    title: 'RMIH - Platform SaaS SDM & Payroll Terintegrasi',
    description:
      'RMIH menyatukan absensi, penggajian, permohonan cuti, onboarding, notifikasi, dan operasional lapangan pada web, mobile, dan kiosk.',
  },
  fr: {
    title: 'RMIH - SaaS RH multilingue pour equipes terrain',
    description:
      'RMIH centralise pointage, paie, absences, onboarding, notifications et operations terrain sur web, mobile et kiosque.',
  },
  en: {
    title: 'RMIH - Multilingual HR SaaS for field teams',
    description:
      'RMIH centralizes attendance, payroll, leave, onboarding, notifications and field operations across web, mobile and kiosk.',
  },
  tr: {
    title: 'RMIH - Saha ekipleri icin cok dilli IK SaaS',
    description:
      'RMIH; yoklama, maaş, izin, onboarding, bildirim ve saha operasyonlarını web, mobil ve kiosk üzerinden merkezileştirir.',
  },
  ar: {
    title: 'RMIH - نظام موارد بشرية سحابي متعدد اللغات للفرق الميدانية',
    description:
      'يجمع RMIH الحضور والرواتب والإجازات والتأهيل والإشعارات والعمليات الميدانية عبر الويب والجوال وجهاز الحضور.',
  },
};
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const landingMeta = pageMetadataI18n[ssrLocale as 'id' | 'en' | 'tr' | 'ar']?.landing;
  const title = landingMeta?.title ?? "RMIH - Platform SaaS SDM & Payroll Terintegrasi";
  ...
  const rootL10n = rootSeoL10n[ssrLocale] ?? rootSeoL10n.id ?? rootSeoL10n.fr;
  ...
  return {
    ...
    openGraph: {
      ...
      siteName: BRAND_NAME_BY_LOCALE[ssrLocale] ?? 'RMIH',
      title,
      description,
      url: siteUrl,
      images: [
...
function resolveSsrLang(acceptLanguage: string | null): string {
  ...
  return ['id', 'fr', 'en', 'ar', 'tr'].includes(base) ? base : 'id';
}
  ...
  return (
    <html lang={ssrLang} dir={resolveSsrDir(ssrLang)} className={inter.variable} suppressHydrationWarning>
      <head>
        ...
        <meta name="apple-mobile-web-app-title" content="RMIH" />
