...
const featuresByLocale: Record<Lang, FeatureCopy[]> = {
  id: [
    { title: 'Logo Perusahaan', desc: 'Unggah logo Anda (PNG, SVG). Tampil di aplikasi mobile, email, dan slip gaji PDF.', color: 'blue' },
    { title: 'Nama Tampilan', desc: 'Sesuaikan nama yang tampil di aplikasi: nama perusahaan Anda bukan ID teknis.', color: 'blue' },
    { title: 'Warna Tema Utama', desc: 'Tentukan warna tema utama dan aksen. Pratinjau langsung di dasbor.', color: 'violet' },
    { title: 'Branding Mobile', desc: 'Tema diterapkan ke aplikasi manajer dan karyawan. Header, tombol, dan lencana mengikuti warna Anda.', color: 'pink' },
    { title: 'Dasbor Admin', desc: 'Portal web manajer mencerminkan identitas visual perusahaan sejak awal login.', color: 'amber' },
    { title: 'Isolasi Multi-Tenant', desc: 'Setiap perusahaan memiliki identitas terpisah. Data dan tema sepenuhnya terisolasi.', color: 'red' },
  ],
  fr: [
    { title: 'Logo entreprise', desc: "Uploadez votre logo (PNG, SVG). Il s'affiche dans l'app mobile, les emails et les bulletins PDF.", color: 'blue' },
  ...
  en: [
    { title: 'Company logo', desc: 'Upload your logo (PNG, SVG). It appears in the mobile app, emails and PDF payslips.', color: 'blue' },
  ...
  tr: [
    { title: 'Şirket logosu', desc: "Logonuzu yükleyin (PNG, SVG). Mobil uygulamada, e-postalarda ve PDF bordrolarda görünür.", color: 'blue' },
  ...
  ar: [
    { title: 'شعار الشركة', desc: 'ارفع شعارك (PNG، SVG). يظهر في التطبيق الجوال والبريد الإلكتروني وكشوف الرواتب PDF.', color: 'blue' },
...
const copy: Record<Lang, BrandingPageCopy> = {
  id: {
    hero: 'Kustomisasi Merek Perusahaan',
    heroSub: 'Jadikan RMIH identik dengan perusahaan Anda. Logo, palet warna, dan nama platform sesuai identitas bisnis Anda.',
    badge: 'Tenant Branding Premium',
    featuresTitle: 'Fitur yang Dapat Disesuaikan',
    apiSectionTitle: 'API Kustomisasi Merek',
    plansTitle: 'Branding Berdasarkan Paket',
    recommendedBadge: 'Direkomendasikan',
    ctaTitle: 'Siap mempersonalisasi ruang kerja Anda?',
    ctaSub: 'Jadwalkan demo untuk melihat implementasi branding premium secara langsung.',
    ctaBtn: 'Mulai Personalisasi',
  },
  fr: {
    hero: 'Personnalisation entreprise',
    heroSub: 'Faites de RMIH votre propre outil. Logo, couleurs, nom — tout reflète votre identité.',
    badge: 'Tenant Branding Premium',
  ...
  en: {
    ...
    heroSub: 'Make RMIH yours. Logo, colours, name — everything reflects your identity.',
    badge: 'Tenant Branding Premium',
  ...
  tr: {
    hero: 'Kurumsal Marka',
    heroSub: 'RMIH\'yi kendinize ait yapın. Logo, renkler, isim — her şey kimliğinizi yansıtır.',
  ...
  ar: {
    ...
    heroSub: 'اجعل RMIH منصتك الخاصة. شعار، ألوان، اسم — كل شيء يعكس هويتك.',
...
export default function BrandingPage() {
  ...
  const t = copy[locale] ?? copy.id ?? copy.fr;
  const features = featuresByLocale[locale] ?? featuresByLocale.id ?? featuresByLocale.fr;
  const plans = plansByLocale[locale] ?? plansByLocale.id ?? plansByLocale.fr;
