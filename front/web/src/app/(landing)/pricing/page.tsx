  ...
  if (value === true) {
    return (
      ...
      >
        <Check
          className={`w-4 h-4 ${
            popular ? 'text-blue-700 dark:text-blue-400' : 'text-slate-500 dark:text-slate-400'
          }`}
        />
      </span>
    );
  ...
  return (
    ...
    >
      ...
      <section className="relative min-h-[60vh] flex items-center justify-center overflow-hidden pt-24 pb-20">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] bg-blue-500/10 rounded-full blur-[100px] animate-pulse [animation-delay:2s]" />
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            <Link
              ...
              className="group relative px-8 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold rounded-2xl overflow-hidden transition-all duration-300 cta-glow-blue hover:scale-[1.03] active:scale-[0.98]"
            >
      ...
      <section className={signupFocus ? 'relative py-10 overflow-hidden' : 'relative py-24 overflow-hidden'}>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* PA2-MKT-003: selecteur de devise. Masque en mode inscription et sur le marche Indonesia (exclusif Rp) */}
          {!signupFocus && locale !== 'id' && (
            <>
          ...
          <div className="flex items-center justify-center gap-4 mb-14">
            ...
            <button
              ...
              className="relative w-16 h-8 rounded-full bg-blue-600 shadow-inner shadow-blue-700/30 transition-colors"
            >
              <motion.div
            ...
            <AnimatePresence>
              {isAnnual && (
                <motion.span
                  ...
                  className="px-3 py-1 text-xs font-black text-blue-700 bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400 rounded-full"
                >
                  {annualSavingsLabel}
                </motion.span>
              )}
              ...
              return (
                <motion.div
                  ...
                  className={`relative rounded-3xl ${
                    plan.popular
                      ? 'bg-gradient-to-b from-blue-500 via-blue-600 to-indigo-600 p-px shadow-2xl shadow-blue-500/25'
                      : isFree
                  ...
                  <div className="relative h-full rounded-[23px] bg-white dark:bg-slate-950 flex flex-col p-8">
                    ...
                    {plan.popular && (
                      <div className="absolute -top-4 left-1/2 -translate-x-1/2 z-10">
                        <div className="flex items-center gap-1.5 px-4 py-1.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-[11px] font-black uppercase tracking-widest rounded-full shadow-lg shadow-blue-500/30">
                    ...
                    <div className="mb-6">
                        ...
                        {isFree ? (
                          ...
                          <>
                            <span className="text-lg font-bold text-blue-600 dark:text-blue-400">
                              {locale === 'id' ? 'Rp' : (isEurSelected ? 'EUR' : currencyOption.currency)}
                            </span>
                            <span className="text-5xl font-black bg-gradient-to-b from-slate-900 to-slate-600 dark:from-white dark:to-slate-400 bg-clip-text text-transparent">
                              {locale === 'id' ? displayPrice : (isEurSelected ? displayPrice : (convertedPrice(displayPrice) ?? displayPrice))}
                            </span>
                          </>
                        ) : (
                      ...
                      {isFree ? (
                        <p className="mt-1 text-sm text-blue-700 dark:text-blue-400 font-semibold">
                          ...
                          {isAnnual && (
                            <p className="text-xs text-slate-500 dark:text-slate-400">
                              <span className="line-through">
                                {locale === 'id' ? `Rp ${plan.price}` : (isEurSelected ? `EUR ${plan.price}` : `${currencyOption.currency} ${convertedPrice(plan.price) ?? plan.price}`)}
                              </span>
                              {' '}
                              <span className="text-blue-700 dark:text-blue-400 font-semibold">{annualSavingsLabel}</span>
                            </p>
                          )}
                          {!isEurSelected && locale !== 'id' && (
                    ...
                    <ul className="flex-1 space-y-3 mb-8">
                      {plan.features.map((feature, fi) => (
                        <li key={fi} className="flex items-start gap-3">
                          <Check className={`w-4 h-4 flex-shrink-0 mt-0.5 ${plan.popular ? 'text-blue-600' : isFree ? 'text-slate-500' : 'text-slate-400'}`} />
                    ...
                    <Link
                      ...
                      className={`flex items-center justify-center gap-2 w-full py-4 rounded-2xl font-bold text-sm transition-all duration-300 ${
                        plan.popular
                          ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:from-blue-700 hover:to-indigo-700 shadow-lg shadow-blue-500/25 hover:shadow-blue-500/40 hover:scale-[1.02] active:scale-[0.98]'
                          : isFree
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-col items-center gap-1.5">
                          ...
                          {plan.name}
                          {plan.popular && (
                            <span className="text-[9px] px-2 py-0.5 bg-blue-600 text-white rounded-full font-black uppercase tracking-wider">
                              ★ top
                            </span>
                          )}
                          {isFree && (
              ...
              <tbody>
                {copy.comparison.categories.map((cat, catIdx) => (
                  <Fragment key={cat.category}>
                    {/* Category row */}
                    <tr className={catIdx % 2 === 0 ? 'bg-transparent/70 dark:bg-slate-900/30' : 'bg-blue-50/30 dark:bg-blue-950/10'}>
                    ...
                    {cat.features.map((feature, fIdx) => (
                      ...
                      >
                        ...
                        {plans.map((plan) => (
                          <td
                            key={plan.name}
                            className={`py-4 px-4 text-center ${plan.popular ? 'bg-blue-50/40 dark:bg-blue-950/10' : ''}`}
                          >
                            <AvailabilityMark
                ...
                <tr className="border-t border-slate-200 dark:border-slate-800 bg-transparent dark:bg-slate-900">
                  ...
                  {plans.map((plan) => {
                    ...
                    return (
                      <td key={plan.name} className={`py-6 px-4 text-center ${plan.popular ? 'bg-blue-50/40 dark:bg-blue-950/10' : ''}`}>
                        <Link
      ...
      <section className="relative py-32 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-blue-500 via-blue-600 to-cyan-600" />
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            <Link
              href="/checkout?plan=free"
              className="group relative px-10 py-4 bg-white text-blue-600 font-black rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:scale-[1.03] active:scale-[0.98] text-base"
            >
              <span className="relative z-10 flex items-center gap-2.5">
                {copy.cta.primary}
                <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
