...
/* ─────────────────────────────────────────
   Copy — 4 languages
───────────────────────────────────────── */
type Lang = 'id' | 'fr' | 'en' | 'tr' | 'ar';
const langs: Lang[] = ['id', 'fr', 'en', 'tr', 'ar'];
...
const copy: Record<Lang, LangCopy> = {
  id: {
    ...
    hero: 'Kelola Perusahaan Langsung dari Genggaman',
    heroSub:
      'Tiga aplikasi Flutter khusus — Employee, Manager, dan Platform Admin — untuk mendukung seluruh peran operasional kerja Anda.',
    appsTitle: 'Tiga Aplikasi, Satu Ekosistem',
    featuresTitle: 'Fitur Unggulan',
    downloadTitle: 'Unduh Aplikasi',
    downloadSub: 'Akses uji coba: aplikasi didistribusikan untuk pengguna pilot terdaftar sebelum rilis publik di toko aplikasi.',
    android: 'Google Play',
    ios: 'App Store',
    soon: 'Segera Hadir',
    beta: 'Beta',
    waitingPrefix: 'Sementara itu, jadwalkan demo atau pelajari',
...
const apps: MobileApp[] = [
  {
    ...
    icon: Clock,
    features: [
      { icon: Clock, label: { id: 'Check-in / Check-out', fr: 'Pointage check-in/out', en: 'Check-in / check-out', tr: 'Giriş/Çıkış', ar: 'تسجيل الدخول/الخروج' } },
      { icon: MapPin, label: { id: 'Geolokasi GPS', fr: 'Géolocalisation GPS', en: 'GPS geolocation', tr: 'GPS konumu', ar: 'تحديد الموقع GPS' } },
      { icon: FileText, label: { id: 'Slip gaji PDF', fr: 'Bulletins de paie PDF', en: 'PDF payslips', tr: 'PDF bordro', ar: 'قسائم الراتب PDF' } },
      { icon: Bell, label: { id: 'Notifikasi push FCM', fr: 'Notifications push FCM', en: 'FCM push notifications', tr: 'FCM bildirimleri', ar: 'إشعارات FCM' } },
      { icon: Zap, label: { id: 'Pengajuan cuti & izin', fr: "Demandes d'absence", en: 'Leave requests', tr: 'İzin talepleri', ar: 'طلبات الإجازات' } },
      { icon: CheckCircle, label: { id: 'Kasbon/Pinjaman', fr: 'Avances salaires', en: 'Salary advances', tr: 'Maaş avansları', ar: 'سلف الراتب' } },
    ],
  },
  {
    id: 'manager',
    ...
    icon: Users,
    features: [
      { icon: Users, label: { id: 'Pantau tim real-time', fr: 'Vue équipe temps réel', en: 'Real-time team view', tr: 'Gerçek zamanlı ekip', ar: 'الفريق في الوقت الحقيقي' } },
      { icon: Clock, label: { id: 'Jadwal & shift kerja', fr: 'Horaires & plannings', en: 'Schedules & planning', tr: 'Programlar', ar: 'الجداول الزمنية' } },
      { icon: CheckCircle, label: { id: 'Persetujuan pengajuan', fr: 'Approbations demandes', en: 'Request approvals', tr: 'Talep onayları', ar: 'الموافقات' } },
      { icon: BriefcaseBusiness, label: { id: 'Tugas harian', fr: 'Tâches journalières', en: 'Daily tasks', tr: 'Günlük görevler', ar: 'المهام اليومية' } },
      { icon: FileText, label: { id: 'Penggajian sederhana', fr: 'Paie simplifiée', en: 'Simplified payroll', tr: 'Basit bordro', ar: 'رواتب مبسطة' } },
      { icon: Bell, label: { id: 'Notifikasi tim', fr: 'Alertes équipe', en: 'Team alerts', tr: 'Ekip uyarıları', ar: 'تنبيهات الفريق' } },
    ],
  },
  {
    id: 'platform-admin',
    ...
    icon: Building2,
    features: [
      { icon: Building2, label: { id: 'Buat tenant baru', fr: 'Création tenants', en: 'Tenant creation', tr: 'Kiracı oluşturma', ar: 'إنشاء المستأجرين' } },
      { icon: Shield, label: { id: 'Autentikasi 2FA', fr: 'Auth 2FA renforcée', en: 'Enhanced 2FA auth', tr: 'Gelişmiş 2FA', ar: 'مصادقة ثنائية' } },
      { icon: Users, label: { id: 'Pengawasan klien', fr: 'Supervision clients', en: 'Client supervision', tr: 'Müşteri denetimi', ar: 'إشراف العملاء' } },
      { icon: Globe, label: { id: 'Isolasi multi-tenant', fr: 'Multi-tenant isolation', en: 'Multi-tenant isolation', tr: 'Çok kiracılı yalıtım', ar: 'عزل متعدد المستأجرين' } },
      { icon: Zap, label: { id: 'Provisioning cepat', fr: 'Provisioning rapide', en: 'Fast provisioning', tr: 'Hızlı provizyon', ar: 'توفير سريع' } },
      { icon: Bell, label: { id: 'Notifikasi platform', fr: 'Alertes plateforme', en: 'Platform alerts', tr: 'Platform uyarıları', ar: 'تنبيهات المنصة' } },
    ],
  },
];
...
export default function MobilePage() {
  ...
  const t = copy[lang] ?? copy.id ?? copy.fr;
      ...
      <section className="pt-32 pb-20 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-wrap justify-center gap-6">
            {[
              { value: '3', label: { id: 'Aplikasi', fr: 'Applications', en: 'Apps', tr: 'Uygulama', ar: 'تطبيقات' } },
              { value: 'Flutter', label: { id: 'Framework', fr: 'Framework', en: 'Framework', tr: 'Framework', ar: 'إطار' } },
              { value: '5', label: { id: 'Bahasa', fr: 'Langues', en: 'Languages', tr: 'Dil', ar: 'لغات' } },
              { value: 'FCM', label: { id: 'Notifikasi', fr: 'Notifications', en: 'Notifications', tr: 'Bildirimler', ar: 'إشعارات' } },
            ].map((stat) => (
                ...
                >
                  ...
                  <div className="w-full lg:w-2/5 flex-shrink-0">
                    <div className={`rounded-3xl border ${c.border} ${c.bg} p-8 flex flex-col items-center text-center shadow-sm`}>
                      ...
                      <p className={`text-xs font-semibold px-3 py-1 rounded-full ${c.badge} mb-4`}>
                        {(app.subtitle[lang] || app.subtitle.id || app.subtitle.fr || app.subtitle.en)}
                      </p>
                      {/* Fake phone mockup */}
                  ...
                  <div className="flex-1">
                    <p className="text-slate-600 dark:text-slate-400 text-base leading-relaxed mb-6">
                      {(app.desc[lang] || app.desc.id || app.desc.fr || app.desc.en)}
                    </p>
                    ...
                    <div className="grid sm:grid-cols-2 gap-3">
                      {app.features.map((feat) => (
                        <div key={feat.label.en} className="flex items-center gap-3">
                          ...
                          <span className="text-sm text-slate-700 dark:text-slate-300">{(feat.label[lang] || feat.label.id || feat.label.fr || feat.label.en)}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="mt-6 flex flex-wrap gap-3">
                      ...
                      <Link
                        href={iosTarget.href}
                        className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:border-blue-400 hover:text-blue-700 transition-colors"
      ...
      <section id="download" className="py-20 px-4">
        ...
        >
          ...
          <p className="mt-8 text-sm text-slate-500 dark:text-slate-400">
            {t.waitingPrefix}{' '}
            <Link href="/docs#sdk-overview" className="text-blue-700 dark:text-blue-400 hover:underline">
              {t.installGuide}
            </Link>
            .
          </p>
