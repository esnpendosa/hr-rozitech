...
const SUBJECT_LABELS: Record<string, Record<SubjectId, string>> = {
  id: {
    ...
    demo: 'Permintaan Demo',
    support: 'Dukungan Teknis',
    partnership: 'Kemitraan',
    press: 'Pers & Media',
    password: 'Lupa Kata Sandi',
    upgrade: 'Tingkatkan Paket',
    'download-kiosk': 'Unduh - Kiosk',
    'download-windows': 'Unduh - Windows',
    'download-macos': 'Unduh - macOS',
...
function ContactPageInner() {
  ...
  const copy = contactCopy[locale] ?? contactCopy.id ?? contactCopy.fr;
  useScrollReveal();
  ...
  useEffect(() => {
    ...
    if (!mappedId) return;
    setForm(prev => ({ ...prev, subject: (SUBJECT_LABELS[locale] || SUBJECT_LABELS.id || SUBJECT_LABELS.fr)[mappedId] }));
  }, [searchParams, locale]);
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid sm:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        >
                          ...
                          {SUBJECT_IDS.map(id => (
                            <option key={id} value={(SUBJECT_LABELS[locale] || SUBJECT_LABELS.id || SUBJECT_LABELS.fr)[id]}>{(SUBJECT_LABELS[locale] || SUBJECT_LABELS.id || SUBJECT_LABELS.fr)[id]}</option>
                          ))}
                        </select>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full px-4 py-3 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-600 focus:border-transparent resize-none"
                      />
                    </div>
                    ...
                    <button
                      ...
                      className="w-full sm:w-auto px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-colors disabled:opacity-50 flex items-center gap-2"
                    >
