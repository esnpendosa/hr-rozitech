...
const copyByLocale: Record<AppLocale, LaunchCopy> = {
  id: {
    badge: 'Peluncuran Pilot dalam 7 Hari',
    title: 'Beralih dari tim yang tersebar ke sistem operasi perusahaan mobile-first.',
    subtitle:
      'RMIH menyatukan absensi, permohonan, manajer, payroll sederhana, dokumen, notifikasi, dan kiosk dalam satu alur kerja siap pakai sejak minggu pertama.',
    primary: 'Mulai Uji Coba Pilot',
    secondary: 'Lihat Aplikasi',
    outcomes: [
      { title: 'Hari 1', detail: 'Perusahaan dibuat, manajer aktif, negara/mata uang/bahasa dikonfigurasi.' },
      { title: 'Hari 2-3', detail: 'Karyawan ditambahkan via formulir atau QR, jadwal dan aturan ditetapkan.' },
      { title: 'Hari 4-5', detail: 'Absensi mobile/kiosk, pengajuan cuti dan kasbon terlihat oleh manajer.' },
      { title: 'Hari 6-7', detail: 'Dashboard, ekspor, notifikasi, dan laporan keputusan pertama siap.' },
    ],
  },
  fr: {
    ...
    subtitle:
      'RMIH rassemble pointage, demandes, managers, paie simple, documents, notifications et kiosk dans un parcours exploitable par vos equipes terrain des la premiere semaine.',
  ...
  en: {
    ...
    subtitle:
      'RMIH brings attendance, requests, managers, simple payroll, documents, notifications and kiosk into one workflow your field teams can use in the first week.',
  ...
  tr: {
    ...
    subtitle:
      'RMIH yoklama, talepler, yoneticiler, basit bordro, belgeler, bildirimler ve kiosku saha ekiplerinizin ilk haftadan kullanabilecegi tek akista birlestirir.',
  ...
  ar: {
    ...
    subtitle:
      'يجمع RMIH الحضور، الطلبات، المديرين، الرواتب المبسطة، الوثائق، الإشعارات والكشك في مسار واحد قابل للاستخدام منذ الأسبوع الأول.',
...
export function LaunchOperatingSystemSection({ locale }: { locale: AppLocale }) {
  const copy = copyByLocale[locale] ?? copyByLocale.id ?? copyByLocale.fr;
    ...
    <section className="relative overflow-hidden px-4 py-20 sm:px-6 lg:px-8" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 rounded-full border border-blue-500/20 bg-blue-500/10 px-4 py-2 text-sm font-bold text-blue-700 dark:text-blue-300">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link
                ...
                className="inline-flex items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-black text-white transition hover:bg-blue-700 dark:bg-blue-600 dark:text-slate-950 dark:hover:bg-blue-500"
              >
                {copy.primary}
              ...
              <Link
                ...
                className="inline-flex items-center justify-center rounded-2xl border border-slate-200 px-5 py-3 text-sm font-bold text-slate-800 transition hover:border-blue-300 hover:bg-blue-50 dark:border-slate-800 dark:text-slate-100 dark:hover:bg-blue-950/30"
              >
                {copy.secondary}
              </Link>
            </div>
            ...
            {copy.outcomes.map((item, index) => {
              ...
              return (
                <motion.div
                  ...
                  className="rounded-3xl border border-slate-200 bg-transparent/80 p-6 shadow-sm transition hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl hover:shadow-blue-950/5 dark:border-slate-800 dark:bg-slate-900/70 dark:hover:border-blue-900"
                >
                  <div className="mb-5 inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-blue-600 text-white shadow-lg shadow-blue-500/20">
