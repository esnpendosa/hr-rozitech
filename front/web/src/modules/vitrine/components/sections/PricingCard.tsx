...
export function PricingCard({
  ...
  return (
    ...
    >
      ...
      {highlighted && (
        <div className="absolute -inset-px rounded-3xl bg-gradient-to-r from-blue-600 to-indigo-600 opacity-0 group-hover:opacity-20 blur-xl transition-opacity duration-500" />
      )}

      <div
        className={`relative h-full rounded-3xl border transition-all duration-300 p-8 flex flex-col ${
          highlighted
            ? 'bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border-blue-200/50 dark:border-blue-800/50 shadow-xl'
            : 'bg-white dark:bg-slate-900/80 border-slate-200/80 dark:border-slate-800/80 group-hover:border-blue-200/50 dark:group-hover:border-blue-800/50 group-hover:shadow-xl'
        }`}
      >
        {/* Badge */}
        {badge && (
          <div className="inline-flex items-center justify-center w-fit px-3 py-1 rounded-full bg-blue-600 text-white text-xs font-bold uppercase tracking-wider mb-4">
            {badge}
          </div>
        )}
        ...
        <Link
          ...
          className={`w-full py-3 px-4 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center gap-2 mb-8 ${
            highlighted
              ? 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-500/25 hover:scale-[1.02]'
