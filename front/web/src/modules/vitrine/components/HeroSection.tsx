...
function deriveCompanyFromEmail(email: string): string {
  ...
  if (!domain || domain.length < 2) {
    return 'Permintaan Uji Coba RMIH'
  }
...
export function QuickTrialEmailForm({ locale, copy }: { locale: AppLocale; copy: QuickTrialCopy }) {
  ...
  return (
    ...
    >
      <form
        onSubmit={onSubmit}
        className="flex flex-col gap-3 rounded-2xl border border-slate-200/80 bg-white/90 p-2 shadow-2xl shadow-blue-500/10 backdrop-blur dark:border-slate-800/80 dark:bg-slate-900/85 sm:flex-row"
      >
        ...
        <button
          ...
          className="inline-flex min-h-14 items-center justify-center rounded-xl bg-blue-600 hover:bg-blue-700 px-6 text-sm font-black text-white shadow-lg shadow-blue-500/20 transition disabled:cursor-not-allowed disabled:opacity-70"
        >
