...
const COPY: Record<AppLocale, SolutionStackCopy> = {
  id: {
    eyebrow: 'Arsitektur Solusi',
    title: 'Satu platform. Fondasi horizontal. Vertikal bisnis Anda.',
    subtitle:
      'Fondasi inti dan modul terintegrasi dibagi di seluruh industri: setiap vertikal bisnis langsung terhubung tanpa harus membangun dari nol.',
    layerPlatform: {
      name: 'Fondasi Inti',
      desc: 'Identitas, multi-tenant, peran & hak akses, audit log, API terbuka.',
    },
    layerHorizontal: {
      name: 'Modul Horizontal',
      desc: '16 modul fungsional terpadu, siap pakai untuk seluruh klien',
    },
    layerVertical: {
      name: 'Vertikal Industri',
      desc: '4 solusi spesifik industri, terhubung ke modul yang sama',
    },
    hint: 'Pilih vertikal industri untuk melihat modul horizontal yang digunakan.',
    alsoPrefix: 'Juga terhubung dengan:',
    canvasAlt:
      'Representasi 3D platform RMIH: fondasi bersama, 16 modul horizontal, dan 4 vertikal bisnis terhubung.',
    horizontals: {
      dashboard: 'Dasbor Utama',
      employees: 'Karyawan',
      attendance: 'Absensi Presensi',
      attendance_geo: 'Absensi GPS Mobile',
      absences: 'Izin & Cuti',
      ...
      billing: 'Penagihan & Invoice',
      integrations: 'Integrasi API',
      marketing: 'Pemasaran & Notifikasi',
  ...
  fr: {
    ...
    canvasAlt:
      'Représentation 3D de la plateforme RMIH : un socle commun, une couche horizontale de 16 briques transverses, et 4 verticales métier branchées dessus.',
    horizontals: {
  ...
  en: {
    ...
    canvasAlt:
      '3D representation of the RMIH platform: a shared foundation, an horizontal layer of 16 cross-cutting building blocks, and 4 industry verticals plugged into it.',
    horizontals: {
      dashboard: 'Dashboard',
      employees: 'Employees',
  ...
  tr: {
    ...
    canvasAlt:
      'RMIH platformunun 3B gösterimi: ortak bir temel, 16 yatay yapı taşından oluşan bir katman ve bunlara bağlı 4 sektörel dikey çözüm.',
    horizontals: {
      dashboard: 'Panel',
  ...
  ar: {
    ...
    canvasAlt:
      'تمثيل ثلاثي الأبعاد لمنصة روزيتك: أساس مشترك، وطبقة أفقية من 16 مكوّنًا، وأربعة حلول قطاعية متصلة بها.',
    horizontals: {
...
export function getSolutionStackCopy(locale: AppLocale): SolutionStackCopy {
  return COPY[locale] ?? COPY.id ?? COPY.fr;
}
...
export const EXTRA_MODULE_LABELS: Record<AppLocale, Record<string, string>> = {
  id: { documents: 'Dokumen', notifications: 'Notifikasi', fleet: 'Armada' },
