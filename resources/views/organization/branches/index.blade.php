...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 text-xs text-gray-500 mb-1">
                ...
                <span>&rsaquo;</span>
                <span class="text-gray-800 font-semibold">Kantor Cabang</span>
            </div>
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Kantor Cabang &amp; Unit Operasional</h1>
            <p class="text-sm text-gray-500 mt-0.5">Kelola seluruh lokasi kantor pusat, cabang, serta unit operasional perusahaan.</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-2xs">
        <form method="GET" action="{{ route('branches.index') }}" class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari nama kantor cabang atau kota..."
            ...
            <div class="flex items-center gap-2">
                ...
                @if(!empty($search))
                <a href="{{ route('branches.index') }}" class="px-3 py-2 text-xs font-semibold text-gray-600 hover:text-gray-900 border border-gray-200 rounded-xl transition">
                    Reset
                </a>
                @endif
            </div>
                    ...
                    <tr class="hover:bg-gray-50/80 transition">
                        <td class="py-3.5 px-4 font-semibold text-gray-900">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <span class="block text-gray-900 font-bold">{{ $b->name }}</span>
                                    <span class="block text-[11px] text-gray-400 font-normal">{{ $b->address ?? 'Alamat belum diisi' }}</span>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="py-3.5 px-4 text-right">
                            <form method="POST" action="{{ route('branches.destroy', $b->id) }}" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kantor cabang ini?')">
                                @csrf
                                @method('DELETE')
                    ...
                    <tr>
                        <td colspan="6" class="py-12 text-center text-gray-400">
                            <div class="max-w-xs mx-auto space-y-2">
                                ...
                                <p class="text-xs text-gray-500 font-medium">Belum ada data kantor cabang terdaftar.</p>
                                <button type="button" onclick="openBranchModal()" class="text-xs text-blue-600 font-bold hover:underline">
...
<!-- Modal Tambah Cabang -->
<div id="modal-branch" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4 hidden">
    <div class="bg-white w-full max-w-lg rounded-2xl shadow-xl border border-gray-100 overflow-hidden transform transition-all">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <h3 class="text-base font-bold text-gray-900">Tambah Kantor Cabang Baru</h3>
            <button type="button" onclick="closeBranchModal()" class="text-gray-400 hover:text-gray-600 transition">
        ...
        <form method="POST" action="{{ route('branches.store') }}" class="p-6 space-y-4">
            @csrf
            <div>
                <label class="block text-xs font-bold text-gray-700 mb-1">Nama Kantor Cabang *</label>
                <input type="text" name="name" required placeholder="Contoh: Kantor Cabang Utama Jakarta"
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <input type="text" name="code" placeholder="Contoh: KC-JKT"
                        class="w-full text-xs p-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none">
                </div>
                <div>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <input type="text" name="province" placeholder="DKI Jakarta"
                        class="w-full text-xs p-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none">
                </div>
            </div>
            <div>
                <label class="block text-xs font-bold text-gray-700 mb-1">Alamat Lengkap</label>
                <textarea name="address" rows="2" placeholder="Jl. Sudirman No..."
            ...
            <div class="pt-3 border-t border-gray-100 flex items-center justify-end gap-2.5">
                <button type="button" onclick="closeBranchModal()" class="px-4 py-2 border border-gray-300 text-gray-700 text-xs font-semibold rounded-xl hover:bg-gray-50 transition">
                    Batal
                </button>
